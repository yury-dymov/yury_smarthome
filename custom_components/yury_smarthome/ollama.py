...
from .entity import LocalLLMClient, TextGenerationResult
...
def _format_url(*, hostname: str, port: str, ssl: bool, path: str = ""):
    return (
        f"{'https' if ssl else 'http'}://{hostname}{':' + port if port else ''}{path}"
    )
...
class OllamaAPIClient(LocalLLMClient):
    ...
    async def send_message(self, model: str, message: str) -> str | None:
        ...
        response = await client.chat(model, messages=messages, stream=False)
        return response.message.content

    @staticmethod
    def get_name(client_options: dict[str, Any]):
        return f"Ollama at '{OllamaAPIClient._api_host(client_options)}'"
    ...
    async def async_validate_connection(
        ...
        if user_input.get(CONF_SSL):
            verify_context = await hass.async_add_executor_job(
                _build_default_ssl_context
            )

        client = AsyncClient(
            host=OllamaAPIClient._api_host(user_input),
            timeout=timeout_config,
    ...
    async def async_get_available_models(self) -> List[str]:
        ...
        try:
            ...
            raise HomeAssistantError(
                "Timed out while fetching models from the Ollama server"
            ) from err
        except (ResponseError, ConnectionError) as err:
            raise HomeAssistantError(
                f"Failed to fetch models from the Ollama server: {err}"
            ) from err
    ...
    def _extract_response(
        self, response_chunk: ChatResponse
    ...
    def _generate_stream(
        ...
        think_mode = entity_options.get(
            CONF_ENABLE_THINK_MODE, DEFAULT_ENABLE_THINK_MODE
        )
        ...
        async def anext_token() -> AsyncGenerator[
            ...
            try:
                format_option = entity_options.get(
                    CONF_RESPONSE_JSON_SCHEMA, "json" if json_mode else None
                )
                ...
                raise HomeAssistantError(
                    f"Failed to communicate with the API! {err}"
                ) from err
